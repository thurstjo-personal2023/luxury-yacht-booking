...
const mockMfaUtilities = {
  ...
  mockPhoneVerification: jest.fn(
    (userId: string, phoneNumber: string): PhoneVerifier => ({
      ...
      sendVerificationCode: jest.fn().mockImplementation(() => Promise.resolve({ verificationId: 'test-verification-id' })),
      verifyCode: jest.fn().mockImplementation(function(code: string) { return Promise.resolve(code === '123456'); }),

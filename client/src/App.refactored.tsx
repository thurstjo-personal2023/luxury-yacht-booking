...
// Import clean architecture services & providers
import { AuthProvider } from "./providers/auth-provider";
import { FirebaseAuthService } from "../infrastructure/adapter/firebase-auth-service";
import { FirebaseAdminAuthService } from "../infrastructure/adapter/firebase-admin-auth-service";
import { FirestoreUserRepository } from "../infrastructure/adapter/firestore-user-repository";
import { FirestoreAdminRepository } from "../infrastructure/adapter/firestore-admin-repository";
import { WouterNavigationService } from "../infrastructure/adapter/wouter-navigation-service";
...
function App() {
  // Initialize services for clean architecture
  const userRepository = new FirestoreUserRepository();
  const adminRepository = new FirestoreAdminRepository();
  const authService = new FirebaseAuthService(userRepository);
  const adminAuthService = new FirebaseAdminAuthService(adminRepository);
  const navigationService = new WouterNavigationService();
          ...
          <main className="flex-1">
            <Suspense fallback={<LoadingSpinner />}>
              <Switch>
                ...
                {/* User Dashboard Routes - Protected with Role Checks */}
                ...
                <Route path="/admin/mfa-setup">
                  <PrivateRoute routeType="admin">
                    <AdminMfaSetup />
                  </PrivateRoute>
                </Route>
                <Route path="/admin/mfa-verify">
                  <PrivateRoute routeType="admin">
                    <AdminMfaVerify />
                  </PrivateRoute>
                </Route>

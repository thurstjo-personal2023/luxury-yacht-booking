...
import { 
  initializeTestEnvironment as emulatorInit,
  EmulatorInstance
} from './emulator-setup';
...
describe('Administrator Registration & Validation', () => {
  ...
  beforeAll(async () => {
    // Initialize the test environment with Firebase emulators
    testEnv = emulatorInit({
    ...
    auth = testEnv.auth as Auth;
    db = testEnv.firestore as Firestore;
